...
def default_pancreas_loader(img_path, mask_path):
...
def read_pancreas_datasets(root_path,mode='train'):
    images = []
    masks = []

    image_root = os.path.join(root_path, 'train/image')
    gt_root = os.path.join(root_path, 'train/label')
...
class ImageFolder(data.Dataset):
    ...
    def __init__(self,root_path, datasets='Messidor',  mode='train'):
        ...
        if self.dataset == 'pancreas':
            self.images, self.labels = read_pancreas_datasets(self.root,self.mode)

    def __getitem__(self, index):

        img, mask = default_pancreas_loader(self.images[index], self.labels[index])
